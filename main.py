...
from sklearn.feature_extraction.text import TfidfVectorizer  # 텍스트 데이터를 TF-IDF 벡터로 변환
...
def train(csv_path: str, model_out: str):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df['text'], df['label'], test_size=0.2, random_state=42, stratify=df['label'].values
    )
    # 학습 데이터 개수 확인
    ...
    pipeline = Pipeline([
        ...
        ('tfidf', TfidfVectorizer(
            ...
            ngram_range=(1,2),  # 1-gram, 2-gram 단어 조합 사용
            max_features=10000,  # 특성을 10000개만 사용(메모리 효율)
            min_df=2,  # 2번 이상 등장하는 단어만 사용(노이즈 제거)
            analyzer='word',  # 단어를 대상으로 분석
        )),

        # 선형 서포트 벡터 머신 분류
        ('clf', LinearSVC(
            C=1.0,  # 규재 강도(수치가 클수록 복잡도가 높아짐)
            random_state=42
        ))
    ])
    ...
    # 6. 성능 테스트(정확도, 정밀도, 재현율, F1-score)
    # digits=4: 소수점 4자리까지 표시
    print(classification_report(y_test, pred, digits=4))

    # 7. 모델 저장 디렉토리 생성
    # exist_ok=True: 디렉토리가 이미 존재할 경우 그냥 실행, 없으면 생성
...
if __name__ == "__main__":
    # 커맨드라인 파라미터 정의
    parser = argparse.ArgumentParser(description="한국어 보이스 피싱 탐지 모델 학습")
    parser.add_argument("--csv", type=str, required=True)
    parser.add_argument("--model_out", type=str, default="models/voice_phishing_model.joblib")

    args = parser.parse_args()

    print("=" * 10)
    print(f'입력 파일: {args.csv}')
    print(f'모델 출력: {args.model_out}')
