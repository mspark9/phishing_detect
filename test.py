...
def predict_text(model, text):
    ...
    Args:
        ...
        text: 예측할 텍스트
    Returns:
        tuple: (예측결과(str), 예측확률(float), 신뢰도, 원본 점수)
    """
    prediction = model.predict([text])[0]  # predict는 2차원 배열을 반환하기 때문에 첫번째 요소 선택
    decision_score = model.decision_function([text])[0]
    # decision_score 예시값: 0.567
    confidence = abs(decision_score)

    # 타겟값에 따른 필터링과 신뢰도 실수에 따른 필터링
    if prediction == 1 :
        result = "보이스피싱"
        confidence_percent = min(0.5 + (decision_score / 2), 1.0) if decision_score > 0 else 0.5
    else:
        result = "정상 텍스트"
        confidence_percent = min(0.5 + (decision_score / 2), 1.0) if decision_score < 0 else 0.5
...
if __name__ == "__main__":
    ...
    # 통장 비밀번호가 필요합니다. 계좌가 묶이지 않으려면 빨리 알려주세요.
